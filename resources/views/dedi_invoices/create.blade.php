...
@section('content')
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-8">
        <h2 class="text-2xl font-bold text-gray-900 mb-6">Buat Nota Faktur H Dedi</h2>

        <div class="bg-white shadow-xl rounded-2xl overflow-hidden border border-gray-100">
            <form action="{{ route('dedi-invoices.store') }}" method="POST" class="p-6 space-y-6">
                @csrf

                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-2">Nomor Invoice (Otomatis)</label>
                        <input type="text" value="{{ $suggestedNumber }}"
                            class="w-full rounded-xl border-gray-300 bg-gray-100 text-gray-500 font-bold cursor-not-allowed"
                            readonly title="Nomor invoice dibuat otomatis">
                        <p class="text-[10px] text-gray-400 mt-1">Nomor ini dibuat otomatis oleh sistem saat disimpan.</p>
                    </div>
                    <div>
