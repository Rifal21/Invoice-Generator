...
@section('title', 'Buat Nota Pengiriman Beras')
...
@section('content')
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-8">
        ...
        <div class="bg-white shadow-2xl rounded-3xl overflow-hidden border-2 border-gray-100">
            <form action="{{ route('rice-deliveries.store') }}" method="POST" class="p-5 sm:p-10 space-y-8 md:space-y-10">
                @csrf
                ...
                <div class="grid grid-cols-1 gap-y-6 gap-x-6 md:grid-cols-4 border-b border-gray-100 pb-8 md:pb-10">
                    <div>
                        ...
                        <div class="relative flex items-center gap-2">
                            ...
                            <button type="button" onclick="toggleManualNota()"
                    ...
                    <div>
                        ...
                        <input type="text" name="location" id="location" value="{{ old('location', 'Ciawi') }}" required
                    ...
                    <div>
                        ...
                        <textarea name="customer_name" id="customer_name" required rows="2"
                            ...
                            placeholder="SPPG Maju Bersama&#10;Pagerageung">{{ old('customer_name') }}</textarea>
                    </div>
                </div>
    ...
    <script>
        ...
        function toggleManualNota() {
            const input = document.getElementById('nota_number');
            if (input.readOnly) {
        ...
        function updateNotaNumber() {
            const input = document.getElementById('nota_number');
            if (!input.readOnly && input.value !== '') return;

            const date = document.getElementById('date').value;
            fetch(`{{ route('rice-deliveries.next-number') }}?date=${date}`)
                .then(response => response.json())
                .then(data => {
                    input.value = data.nota_number;
                });
        }
        ...
        function extractNumeric(str) {
            ...
            // Handle Indonesian format where . is thousand and , is decimal
            ...
            if (cleaned.includes(',') && cleaned.includes('.')) {
                ...
                // If it ends with .XXX and has more than 5 chars, likely a thousand separator
                // This is a bit of a guess, but common in Indo.
                // Better: if there are multiple dots, they are thousands.
                if ((cleaned.match(/\./g) || []).length > 1) {
                    cleaned = cleaned.replace(/\./g, '');
                }
                // If only one dot, it could be decimal or thousand (e.g. 1.000 vs 1.5)
                // If it's exactly 3 digits after dot, we'll assume thousand for large numbers
                else if (parseFloat(cleaned.replace(/\./g, '')) > 100) {
                    // cleaned = cleaned.replace(/\./g, ''); // risky
                }
            }
