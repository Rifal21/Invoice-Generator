...
@section('title', 'Daftar Invoice Sewa Kendaraan')
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
            <div class="min-w-0 flex-1">
                <h1 class="text-3xl md:text-4xl font-black text-gray-900 tracking-tight">Invoice Sewa Kendaraan</h1>
                <p class="mt-2 text-sm md:text-lg text-gray-500">Kelola dan pantau semua invoice sewa kendaraan Anda.</p>
            </div>
            <div class="flex shrink-0">
                <a href="{{ route('vehicle-rentals.create') }}"
        ...
        <!-- Filter Section -->
        <form id="filter-form" action="{{ route('vehicle-rentals.index') }}" method="GET"
            ...
            <div class="grid grid-cols-1 md:grid-cols-12 gap-y-4 gap-x-6 items-end">
                ...
                <div class="md:col-span-6">
                    ...
                    <input type="text" name="search" id="search" value="{{ request('search') }}"
                        placeholder="No. Invoice / Nama Penerima..."
                            ...
                            <tr class="hover:bg-indigo-50/30 transition-colors duration-150 group">
                                <td class="whitespace-nowrap px-6 py-5">
                                    <div class="flex items-center">
                                        <div
                                            class="h-10 w-10 flex-shrink-0 rounded-xl bg-amber-100 flex items-center justify-center text-amber-600">
                                ...
                                <td class="px-3 py-5">
                                    <div class="text-sm font-bold text-gray-900 line-clamp-1">
                                        {{ Str::limit($invoice->customer_name, 50) }}</div>
                                </td>
                            ...
                            <tr>
                                <td colspan="5" class="py-20 text-center">
                                    <div class="flex flex-col items-center">
                                        <i class="fas fa-car-side text-5xl text-gray-200 mb-4"></i>
                                        <p class="text-gray-500 text-lg font-medium">Belum ada invoice sewa kendaraan</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
        ...
        <div class="md:hidden space-y-4 mb-6">
            @forelse($invoices as $invoice)
                <div class="bg-white rounded-3xl p-5 shadow-lg border border-gray-100 relative overflow-hidden">
                    <div class="flex items-start gap-4 mb-4">
                        <div
                            class="h-12 w-12 shrink-0 rounded-2xl bg-amber-100 flex items-center justify-center text-amber-600">
                            <i class="fas fa-car-side"></i>
                        </div>
                        <div>
                    ...
                    <div class="space-y-3 mb-6">
                        <div>
                            ...
                            <p class="text-base font-bold text-gray-900 line-clamp-2">{{ $invoice->customer_name }}</p>
                        </div>
                        <div>
            ...
            @empty
                <div class="text-center py-12 bg-white rounded-3xl border-2 border-dashed border-gray-200">
                    <i class="fas fa-car-side text-4xl text-gray-200 mb-3"></i>
                    <p class="text-gray-500 font-medium">Belum ada invoice sewa kendaraan</p>
                </div>
            @endforelse
        </div>
        ...
        function deleteInvoice(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    form.action = `/vehicle-rentals/${id}`;
                    form.submit();
                }
            })
        }
