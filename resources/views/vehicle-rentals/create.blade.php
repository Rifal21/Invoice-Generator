...
@section('title', 'Buat Invoice Sewa Kendaraan')
...
@section('content')
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-4">
            <div class="min-w-0 flex-1">
                ...
                <h2 class="text-2xl md:text-3xl font-extrabold leading-7 text-gray-900 sm:truncate sm:tracking-tight">Buat
                    Invoice Sewa Kendaraan</h2>
            </div>
        </div>

        <div class="bg-white shadow-2xl rounded-3xl overflow-hidden border-2 border-gray-100">
            <form action="{{ route('vehicle-rentals.store') }}" method="POST" class="p-5 sm:p-10 space-y-8 md:space-y-10">
                @csrf

                <!-- Invoice Details Section -->
                <div class="grid grid-cols-1 gap-y-6 gap-x-6 md:grid-cols-4 border-b border-gray-100 pb-8 md:pb-10">
                    <div>
                        <label for="invoice_number_display" class="block text-sm font-bold text-gray-700 mb-2">No.
                            Invoice</label>
                        <div class="relative">
                            <input type="text" id="invoice_number_display" readonly
                                class="block w-full rounded-2xl border-2 border-gray-100 bg-gray-50 py-3 px-3 text-gray-500 shadow-sm focus:outline-none transition-all duration-200 font-mono font-bold"
                    ...
                    <div>
                        ...
                        <textarea name="customer_name" id="customer_name" required rows="2"
                            ...
                            placeholder="Contoh:&#10;SPPG Dapur Kabungah Cipondok&#10;Kabupaten Tasikmalaya">{{ old('customer_name') }}</textarea>
                    </div>
                </div>
                ...
                <div>
                    <div
                        ...
                        <span class="text-xs font-medium text-gray-400 uppercase tracking-widest">Tambah rincian sewa
                            kendaraan</span>
                    </div>
    ...
    <script>
        ...
        function updateInvoiceNumber() {
            ...
            fetch(`{{ route('vehicle-rentals.next-number') }}?date=${dateInput}`)
                .then(response => response.json())
                .then(data => {
                    document.getElementById('invoice_number_display').value = data.invoice_number;
                });
        }
        ...
        function calculateDays(element) {
            ...
            if (startInput && endInput) {
                ...
                // Hitung selisih hari (+1 untuk menyertakan hari pertama)
        ...
        document.addEventListener('DOMContentLoaded', function() {
            document.getElementById('date').addEventListener('change', updateInvoiceNumber);
            updateInvoiceNumber();
            addItem();
        });
    </script>
