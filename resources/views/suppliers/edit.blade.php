...
@section('content')
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div class="mb-6">
            <nav class="flex mb-4" aria-label="Breadcrumb">
                <ol class="flex items-center space-x-2">
                    <li><a href="{{ route('suppliers.index') }}"
                            class="text-sm font-bold text-gray-400 hover:text-indigo-600">Supplier</a></li>
            ...
            <h1 class="text-3xl font-black text-gray-900 tracking-tight">Edit Supplier</h1>
            <p class="mt-2 text-sm text-gray-500">Perbarui data supplier.</p>
        </div>

        <div class="bg-white shadow-xl rounded-3xl overflow-hidden border border-gray-100">
            <form action="{{ route('suppliers.update', $supplier) }}" method="POST" class="p-6 md:p-8 space-y-6">
                @csrf
                @method('PUT')

                <div>
                    <label for="name" class="block text-sm font-bold text-gray-700 mb-2">Nama Supplier <span
                ...
                <div>
                    <label for="contact_person" class="block text-sm font-bold text-gray-700 mb-2">Kontak Person
                        (PIC)</label>
                ...
                <div>
                    ...
                    <textarea name="address" id="address" rows="3"
                        class="block w-full rounded-2xl border-gray-200 py-3 px-4 text-gray-900 shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all">{{ old('address', $supplier->address) }}</textarea>
                    @error('address')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
