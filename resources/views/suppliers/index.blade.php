    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
            <div class="min-w-0 flex-1">
                <h1 class="text-3xl md:text-4xl font-black text-gray-900 tracking-tight">Daftar Supplier</h1>
                <p class="mt-2 text-sm md:text-lg text-gray-500">Kelola data supplier dan pemasok.</p>
            </div>
            ...
            <form action="{{ route('suppliers.index') }}" method="GET"
                ...
                <div class="md:col-span-10">
                    ...
                    <input type="text" name="search" id="search" value="{{ request('search') }}"
                        placeholder="Nama Supplier, Kontak Person, Telepon..."
                        ...
                        @forelse ($suppliers as $supplier)
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <div class="text-xs text-gray-500 line-clamp-1">{{ $supplier->address }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900">{{ $supplier->contact_person ?? '-' }}
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900">{{ $supplier->phone ?? '-' }}</div>
                                    <div class="text-xs text-indigo-500">{{ $supplier->email ?? '-' }}</div>
                                </td>
