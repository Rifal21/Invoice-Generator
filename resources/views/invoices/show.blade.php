        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-8 md:mb-10 gap-4 md:gap-6">
                ...
                <nav class="flex mb-4" aria-label="Breadcrumb">
                    <ol class="flex items-center space-x-2">
                        <li><a href="{{ route('invoices.index') }}"
                                class="text-xs md:text-sm font-bold text-indigo-600 hover:text-indigo-500">Invoice</a></li>
            ...
            <div class="flex flex-wrap gap-2 md:gap-3">
                <a href="{{ route('invoices.edit', $invoice->id) }}"
                    class="inline-flex items-center justify-center flex-1 md:flex-none px-4 py-2.5 rounded-2xl bg-amber-50 text-amber-700 text-sm font-bold shadow-sm hover:bg-amber-100 transition-all">
                ...
                <a href="{{ route('invoices.export-pdf', $invoice->id) }}" target="_blank"
                    class="inline-flex items-center justify-center flex-1 md:flex-none px-4 py-2.5 rounded-2xl bg-red-50 text-red-700 text-sm font-bold shadow-sm hover:bg-red-100 transition-all">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <a href="{{ route('invoices.export-excel', $invoice->id) }}"
                    class="inline-flex items-center justify-center flex-1 md:flex-none px-4 py-2.5 rounded-2xl bg-green-50 text-green-700 text-sm font-bold shadow-sm hover:bg-green-100 transition-all">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <a href="{{ route('invoices.index') }}"
                    class="inline-flex items-center justify-center w-full md:w-auto px-5 py-2.5 rounded-2xl bg-indigo-600 text-white text-sm font-bold shadow-xl hover:bg-indigo-700 transition-all transform hover:-translate-y-0.5">
                    Kembali
                </a>
            </div>
        </div>
            ...
            <div class="p-6 md:p-12">
                ...
                <div
                    ...
                    <div class="max-w-sm w-full">
                        <div
                            class="h-12 w-12 md:h-16 md:w-16 bg-indigo-600 rounded-2xl flex items-center justify-center text-white mb-6 shadow-lg">
                        ...
                        <h1 class="text-lg md:text-xl font-black text-gray-900 uppercase tracking-tight leading-snug">
                            KOPERASI KONSUMEN JEMBAR RAHAYU SEJAHTERA</h1>
                    ...
                    <div class="text-left md:text-right w-full md:w-auto">
                        <h2 class="text-4xl md:text-5xl font-black text-indigo-600 tracking-tighter uppercase">INVOICE</h2>
                        <div
                            class="mt-6 flex flex-row md:flex-col justify-between md:justify-start items-center md:items-end gap-x-8 gap-y-2">
                            <div>
                                <p
                                    class="text-xs md:text-sm font-bold text-gray-400 uppercase tracking-widest text-left md:text-right">
                                    Nomor Invoice</p>
                                <p class="text-lg md:text-xl font-black text-gray-900 text-left md:text-right">
                                    {{ $invoice->invoice_number }}</p>
                            </div>

                            <div>
                                <p
                                    class="text-xs md:text-sm font-bold text-gray-400 uppercase tracking-widest text-left md:text-right mt-0 md:mt-4">
                                    Tanggal Terbit</p>
                                <p class="text-base md:text-lg font-bold text-gray-900 text-left md:text-right">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 mt-8 md:mt-12 mb-8 md:mb-12">
                    ...
                    <div class="bg-indigo-600 rounded-3xl p-6 md:p-8 text-white shadow-xl transform md:rotate-1">
                        ...
                        <p class="text-3xl md:text-4xl font-black">Rp
                ...
                <!-- Items Table (Desktop) -->
                <div class="hidden md:block overflow-hidden rounded-3xl border border-gray-100 shadow-sm mb-12">
                ...
                <div class="md:hidden space-y-4 mb-8">
                    <h3 class="text-sm font-black text-gray-400 uppercase tracking-widest mb-4">Rincian Item</h3>
                    @foreach ($invoice->items as $index => $item)
                        <div class="bg-gray-50 rounded-2xl p-5 border border-gray-100">
                            ...
                            <div class="flex justify-between items-center text-sm text-gray-600 mb-3">
                                <span>{{ $item->quantity }} {{ $item->unit }} x Rp
                                    {{ number_format($item->price, 0, ',', '.') }}</span>
                            </div>
                            <div class="pt-3 border-t border-gray-200 flex justify-between items-center">
                                <span class="text-xs font-bold text-gray-400 uppercase">Subtotal</span>
                                <span class="text-base font-black text-indigo-600">Rp
                                    {{ number_format($item->total, 2, ',', '.') }}</span>
                            </div>
                        </div>
                    @endforeach

                    <div class="bg-indigo-50 rounded-2xl p-5 border border-indigo-100 mt-4">
                        <div class="flex justify-between items-center">
                            <span class="text-sm font-bold text-indigo-900 uppercase tracking-widest">Total Akhir</span>
